...
enum MazeSize { SIZE_SMALL = 20, SIZE_MEDIUM = 30, SIZE_LARGE = 40 };
...
public:
    ~LinkedList() { clear(); }
    void add(const PlayerMove& m) { LinkedListNode* n = new LinkedListNode(m); if (!head) head = tail = n; else { tail->next = n; tail = n; } ++sz; }
...
class ScoreBST {
    ...
    BSTNode* insert(BSTNode* node, const PlayerScore& p) {
        ...
        if (p < node->data) node->left = insert(node->left, p);
    ...
    void destroy(BSTNode* n) { if (!n) return; destroy(n->left); destroy(n->right); delete n; }
public:
    ~ScoreBST() { clear(); }
    void add(const PlayerScore& p) { root = insert(root, p); }
    vector<PlayerScore> getSorted() { vector<PlayerScore> out; inorder(root, out); return out; }
...
class VisitHashTable {
    ...
    vector<Node*> table;
    int h(int x, int y) const { unsigned long long v = ((unsigned long long)(unsigned)x << 32) ^ (unsigned)y; return static_cast<int>(v % SIZE); }
public:
    VisitHashTable() : table(SIZE, nullptr) {}
    ~VisitHashTable() { clear(); }
    void mark(int x, int y) { int idx = h(x, y); Node* n = new Node(x, y); n->next = table[idx]; table[idx] = n; }
    bool isVisited(int x, int y) const {
        int idx = h(x, y); Node* p = table[idx];
        while (p) { if (p->x == x && p->y == y) return true; p = p->next; }
        return false;
    }
    void clear() {
...
struct Button {
    ...
    bool checkHover(Vector2 m) { hovered = CheckCollisionPointRec(m, rect); return hovered; }
...
vector<Coord> replayPath; int replayIndex = 0; float replayTimer = 0.0f;
...
string CurrentDateTime() {
    ...
    char buf[80]; strftime(buf, 80, "%Y-%m-%d %H:%M:%S", &t); return string(buf);
#else
    tm* t = localtime(&now);
    char buf[80]; strftime(buf, 80, "%Y-%m-%d %H:%M:%S", t); return string(buf);
#endif
...
void initMazeGrid(int rows, int cols) {
    ...
    for (int y = 0; y < rows; ++y) for (int x = 0; x < cols; ++x) maze[y][x] = Cell(x, y);
...
void generateFullMaze(int rows, int cols) {
    ...
    while (!dfsStack.empty()) {
        ...
        for (int d = 0; d < 4; ++d) {
            ...
            if (nx >= 0 && nx < mazeCols && ny >= 0 && ny < mazeRows && !visitedTable.isVisited(nx, ny)) dirs.push_back(d);
        }
        if (!dirs.empty()) {
            std::shuffle(dirs.begin(), dirs.end(), rng);
            int d = dirs[0];
            int nx = cur->x + dx[d], ny = cur->y + dy[d];
            if (d == 0) { cur->walls[0] = false; maze[ny][nx].walls[1] = false; }
            else if (d == 1) { cur->walls[1] = false; maze[ny][nx].walls[0] = false; }
            else if (d == 2) { cur->walls[2] = false; maze[ny][nx].walls[3] = false; }
            else { cur->walls[3] = false; maze[ny][nx].walls[2] = false; }
...
vector<Cell*> BFSSolve() {
    ...
    Queue<Cell*> q;
    q.push(&maze[0][0]); vis[0][0] = true;
    while (!q.empty()) {
        Cell* cur = q.front(); q.pop();
        if (cur->x == mazeCols - 1 && cur->y == mazeRows - 1) break;
        if (!cur->walls[0] && cur->y > 0 && !vis[cur->y - 1][cur->x]) { vis[cur->y - 1][cur->x] = true; parent[cur->y - 1][cur->x] = cur; q.push(&maze[cur->y - 1][cur->x]); }
        if (!cur->walls[1] && cur->y < mazeRows - 1 && !vis[cur->y + 1][cur->x]) { vis[cur->y + 1][cur->x] = true; parent[cur->y + 1][cur->x] = cur; q.push(&maze[cur->y + 1][cur->x]); }
        if (!cur->walls[2] && cur->x > 0 && !vis[cur->y][cur->x - 1]) { vis[cur->y][cur->x - 1] = true; parent[cur->y][cur->x - 1] = cur; q.push(&maze[cur->y][cur->x - 1]); }
        if (!cur->walls[3] && cur->x < mazeCols - 1 && !vis[cur->y][cur->x + 1]) { vis[cur->y][cur->x + 1] = true; parent[cur->y][cur->x + 1] = cur; q.push(&maze[cur->y][cur->x + 1]); }
    }
    Cell* cur = &maze[mazeRows - 1][mazeCols - 1];
    if (!parent[cur->y][cur->x] && !(cur->x == 0 && cur->y == 0)) return path;
    while (cur) {
        path.push_back(cur);
...
void LoadScoresFromFile() {
    ...
    while (getline(f, line)) {
        ...
        if (!getline(ss, diffS, ',')) continue;
        if (!getline(ss, date)) continue;
        try {
            float t = stof(timeS); int di = stoi(diffS);
            PlayerScore ps(name, t, (DifficultyLevel)di, date);
            if (ps.diff == DIFF_EASY) easyScores.add(ps);
            else if (ps.diff == DIFF_MEDIUM) mediumScores.add(ps);
            else hardScores.add(ps);
        }
        catch (...) { continue; }
    }
    f.close();
...
void DrawMaze() {
    ...
    for (int y = 0; y < mazeRows; ++y) {
        for (int x = 0; x < mazeCols; ++x) {
            float cx = startX + x * cellSize;
            float cy = startY + y * cellSize;
            if (maze[y][x].walls[0]) DrawGreenWall(cx, cy - wallThickness / 2.0f, cellSize, wallThickness);
            if (maze[y][x].walls[1]) DrawGreenWall(cx, cy + cellSize - wallThickness / 2.0f, cellSize, wallThickness);
            if (maze[y][x].walls[2]) DrawGreenWall(cx - wallThickness / 2.0f, cy, wallThickness, cellSize);
            if (maze[y][x].walls[3]) DrawGreenWall(cx + cellSize - wallThickness / 2.0f, cy, wallThickness, cellSize);
        }
    }
...
void UpdateHoversMain() {
    ...
    else if (currentState == STATE_SIZE_SELECT) for (auto& b : sizeButtons) b.checkHover(m);
    else if (currentState == STATE_DIFFICULTY_SELECT) for (auto& b : diffButtons) b.checkHover(m);
    else if (currentState == STATE_END && !endButtons.empty())
        for (auto& b : endButtons) b.checkHover(m);
...
void ShowEndComparison() {
    ...
    if (playerPath.getSize() == static_cast<int>(solutionPath.size())) {
        ...
        for (size_t i = 0; i < solutionPath.size() && n; ++i) {
            ...
            if (n->data.x != pos.x || n->data.y != pos.y) { perfect = false; break; }
...
void ResetGameWithCurrentMaze() {
    ...
    playerX = 0; playerY = 0; movesCount = 0; gameTimer = 0.0f; countdownTimer = 3.0f; gamePaused = false;
    playerPath.clear(); playerPath.add(PlayerMove(playerX, playerY, 0.0f));
    solutionPath.clear();
    for (auto* c : BFSSolve()) {
...
float evaluateDifficulty(int rows, int cols) {
    ...
    for (int y = 0; y < R; ++y) for (int x = 0; x < C; ++x) {
        int open = 0; for (int k = 0; k < 4; ++k) if (!maze[y][x].walls[k]) open++;
        if (open == 1) deadEnds++;
        if (open >= 3) branches++;
        if (x + y > maxCoord) maxCoord = x + y;
    }
...
void GenerateMultipleMazesAndPick() {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int y = 0; y < mazeRows; ++y) for (int x = 0; x < mazeCols; ++x) {
            unsigned char b = 0;
            if (maze[y][x].walls[0]) b |= 1 << 0;
            if (maze[y][x].walls[1]) b |= 1 << 1;
            if (maze[y][x].walls[2]) b |= 1 << 2;
            if (maze[y][x].walls[3]) b |= 1 << 3;
            if (maze[y][x].isPath)  b |= 1 << 4;
            md.cellBytes[y * mazeCols + x] = b;
        }
        generatedMazes.push_back(md);
    }
    float minS = 0, maxS = 100;
    if (currentDifficulty == DIFF_EASY) { minS = 0; maxS = 33; }
    else if (currentDifficulty == DIFF_MEDIUM) { minS = 34; maxS = 66; }
    else { minS = 67; maxS = 100; }
    sort(generatedMazes.begin(), generatedMazes.end(), [](const MazeDifficulty& a, const MazeDifficulty& b) { return a.score > b.score; });
    bool found = false;
    for (auto& md : generatedMazes) {
        if (md.score >= minS && md.score <= maxS) {
            maze.clear(); maze.resize(md.rows, vector<Cell>(md.cols));
            for (int y = 0; y < md.rows; ++y) for (int x = 0; x < md.cols; ++x) {
                maze[y][x] = Cell(x, y);
...
int main() {
    ...
    while (!WindowShouldClose() && !shouldClose) {
        ...
        case STATE_HOME: {
            Vector2 m = GetMousePosition();
            if (homeButtons[0].clicked(m)) { currentState = STATE_SIZE_SELECT; }
            if (homeButtons[1].clicked(m)) { currentState = STATE_SCOREBOARD; }
        ...
        case STATE_COUNTDOWN: {
            countdownTimer -= dt;
            if (countdownTimer <= 0.0f) { currentState = STATE_PLAYING; ResetGameWithCurrentMaze(); }
            break;
        }
        case STATE_PLAYING: {
            if (!gamePaused) {
                gameTimer += dt;
                if (IsKeyPressed(KEY_UP) && playerY > 0 && !maze[playerY][playerX].walls[0]) { playerY--; movesCount++; playerPath.add(PlayerMove(playerX, playerY, gameTimer)); }
                if (IsKeyPressed(KEY_DOWN) && playerY < mazeRows - 1 && !maze[playerY][playerX].walls[1]) { playerY++; movesCount++; playerPath.add(PlayerMove(playerX, playerY, gameTimer)); }
                if (IsKeyPressed(KEY_LEFT) && playerX > 0 && !maze[playerY][playerX].walls[2]) { playerX--; movesCount++; playerPath.add(PlayerMove(playerX, playerY, gameTimer)); }
                if (IsKeyPressed(KEY_RIGHT) && playerX < mazeCols - 1 && !maze[playerY][playerX].walls[3]) { playerX++; movesCount++; playerPath.add(PlayerMove(playerX, playerY, gameTimer)); }

                if (playerX == mazeCols - 1 && playerY == mazeRows - 1) {
                    gameCompleted = true;
                    PlayerScore ps(playerName, gameTimer, currentDifficulty, CurrentDateTime());
                    if (currentDifficulty == DIFF_EASY) easyScores.add(ps);
                    else if (currentDifficulty == DIFF_MEDIUM) mediumScores.add(ps);
                    else hardScores.add(ps);
                    SaveScoreToFile(ps);
        ...
        case STATE_END: {
            ...
            else if (endButtons[2].clicked(m)) { currentState = STATE_HOME; }
            break;
        }
        ...
        if (currentState == STATE_HOME) {
            ...
            DrawMaze();
            DrawPlayer();
            if (countdownTimer > 2.0f) DrawCenteredTextShadow("3", (SCREEN_WIDTH - UI_PANEL_WIDTH) / 2, SCREEN_HEIGHT / 2 - 40, 120, RED);
            else if (countdownTimer > 1.0f) DrawCenteredTextShadow("2", (SCREEN_WIDTH - UI_PANEL_WIDTH) / 2, SCREEN_HEIGHT / 2 - 40, 120, MY_ORANGE);
            else if (countdownTimer > 0.0f) DrawCenteredTextShadow("1", (SCREEN_WIDTH - UI_PANEL_WIDTH) / 2, SCREEN_HEIGHT / 2 - 40, 120, YELLOW);
            ...
            if (gamePaused) DrawCenteredTextShadow("PAUSED", (SCREEN_WIDTH - UI_PANEL_WIDTH) / 2, 30, 32, RED);
        }
            ...
            if (currentDifficulty == DIFF_EASY) scores = easyScores.getSorted();
            else if (currentDifficulty == DIFF_MEDIUM) scores = mediumScores.getSorted();
            else scores = hardScores.getSorted();
            if (!scores.empty()) {
                int startY = 270; int cnt = min(static_cast<int>(scores.size()), 10);
                DrawText("RANK", 220, 240, 18, BLACK); DrawText("NAME", 320, 240, 18, BLACK); DrawText("TIME", 520, 240, 18, BLACK); DrawText("DATE", 620, 240, 18, BLACK);
                for (int i = 0; i < cnt; ++i) {
                    int y = startY + i * 28;
